...
class TestGenerator:
    ...
    def mock_plugin_manager(self):
        ...
        with patch("ansible_waldur_generator.generator.PluginManager") as mock_pm_class:
            ...
            mock_plugin.generate.return_value = GenerationContext(
                ...
                documentation={
                    "module": "test_module",
                    "description": "Test module documentation",
                },
    ...
    def test_generate_collections(
        self, sample_config, sample_api_spec, temp_output_dir, mock_plugin_manager
    ):
        ...
        collection_path = (
            Path(temp_output_dir) / "ansible_collections" / "test" / "collection"
        )
    ...
    def test_multiple_collections(
        self, sample_api_spec, temp_output_dir, mock_plugin_manager
    ):
    ...
    def test_setup_collection_skeleton(
        self, sample_config, sample_api_spec, temp_output_dir, mock_plugin_manager
    ):
        ...
        for item in os.listdir(temp_output_dir):
            if "ansible_collections" in item or os.path.isdir(
                os.path.join(temp_output_dir, item)
            ):
